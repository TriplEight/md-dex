...
export class BlockchainListenerService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    const blockchainEnabled = this.configService.get<boolean>('blockchain.enabled', false);
    const listenerEnabled = this.configService.get<boolean>('listener.enabled', false);

    if (!blockchainEnabled || !listenerEnabled) {
      this.logger.warn('⚠️  Blockchain listener disabled by config');
      return;
    }
  ...
  async startListening(): Promise<void> {
    ...
    try {
      ...
      if (!factoryAddress) {
        this.logger.warn('Factory address not configured, listener disabled');
        return;
      }
