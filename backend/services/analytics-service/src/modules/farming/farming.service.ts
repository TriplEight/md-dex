...
export class FarmingService {
  ...
  private readonly farmingEnabled: boolean;
  ...
  constructor(
    ...
    this.farmingEnabled =
      this.configService.get<boolean>('blockchain.enabled', false) &&
      this.configService.get<boolean>('farming.enabled', false);
    ...
    if (!this.farmingEnabled) {
      this.logger.warn('Farming features disabled by config');
    } else if (!this.masterChefAddress) {
  ...
  async getAllFarms(): Promise<FarmsResponseDto> {
    ...
    if (!this.farmingEnabled || !this.masterChefAddress) {
      return this.buildEmptyFarmsResponse();
    }
  ...
  async getFarm(poolId: number): Promise<FarmDto> {
    if (!this.farmingEnabled || !this.masterChefAddress) {
      return this.buildEmptyFarm(poolId);
    }
  ...
  async syncPoolFromChain(poolId: number): Promise<Farm> {
    ...
    if (!this.farmingEnabled || !this.masterChefAddress) {
      this.logger.warn(`Farming disabled, skip sync for pool ${poolId}`);
      return this.farmRepository.create({ poolId });
    }
  ...
  private buildEmptyFarmsResponse(): FarmsResponseDto {
    return {
      farms: [],
      summary: {
        totalPools: 0,
  ...
  private buildEmptyFarm(poolId: number): FarmDto {
    return {
      poolId,
      lpToken: {
        address: '',
