...
export class FarmingListenerService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    const farmingEnabled = this.configService.get<boolean>('farming.enabled', false);

    if (!blockchainEnabled || !farmingEnabled) {
      this.logger.warn('⚠️  Farming listener disabled by config');
      return;
    }
