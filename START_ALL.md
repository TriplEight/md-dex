...
### 🧩 Workspace 快速启动（前端 + 后端）
在项目根目录执行一次依赖安装后，可一键启动前端和后端服务：

```bash
# 只需要执行一次（workspace 安装）
npm install

# 一键启动前端 + 后端（Analytics + Wallet）
npm run dev
```

如需完整链路（含 Hardhat、本地部署、同步数据），请继续使用下方方案 A。
...
#### Terminal 3: 后端服务
```bash
cd /dex/backend/services/analytics-service
npm run start:dev
```
**状态：** 保持运行 🟢  
**预期日志：**
...
sleep 5
tmux new-session -d -s dex-backend 'cd backend/services/analytics-service && npm run start:dev'
sleep 3
tmux new-session -d -s dex-frontend 'cd frontend/web-app && npm run dev'
